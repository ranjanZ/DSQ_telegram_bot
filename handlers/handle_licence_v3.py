...
from config import GITHUB_TOKEN, GITHUB_API_URL_V3, BRANCH
...
# ==================== GITHUB UPDATE ====================
async def update_github_licence_v3(mt5_id: str, name: str, broker: str, account_type: str) -> dict:
    headers = {
        "Authorization": f"token {GITHUB_TOKEN}",
        "Accept": "application/vnd.github.v3+json"
    }
    async with httpx.AsyncClient(timeout=20.0) as client:
        resp = await client.get(GITHUB_API_URL_V3, headers=headers, params={"ref": BRANCH})
        ...
        payload = {
            "message": f"Add V3 user {mt5_id} ({name})",
            "content": new_content_b64,
            "sha": sha,
            "branch": BRANCH
        }
        resp = await client.put(GITHUB_API_URL_V3, headers=headers, json=payload)
        resp.raise_for_status()
        return resp.json()
...
async def timeout(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if lang == "hi":
        msg = "⌛ आपने बहुत अधिक समय लिया। कृपया /get_licence_v3 से पुनः प्रारंभ करें।"
    else:
        msg = "⌛ You took too long. Please start again with /get_licence_v3."
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for key in ("licence_mt5", "licence_name", "licence_broker"):
        context.user_data.pop(key, None)
    if lang == "hi":
        msg = "❌ प्रक्रिया रद्द कर दी गई। /get_licence_v3 से पुनः प्रारंभ करें।"
    else:
        msg = "❌ Process cancelled. Use /get_licence_v3 to start again."
...
async def licence_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if lang == "hi":
        ...
        msg = (
            "📋 <b>V3 Licence Activation – Step 1</b>\n\n"
...
async def ask_account_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if lang == "hi":
        progress_text = f"⏳ {tg_first} जी, आपका V3 लाइसेंस अपडेट हो रहा है... कृपया प्रतीक्षा करें।"
    else:
        progress_text = f"⏳ {tg_first}, your V3 licence is being updated... Please wait."
    progress_msg = await update.message.reply_text(progress_text)

    try:
        await update_github_licence_v3(mt5, name, broker, account_type)
        if lang == "hi":
            final_msg = (
                f"✅ धन्यवाद, {name}! आपके V3 बॉट के लिए लाइसेंस बनाया गया है।\n"
                ...
                "2. Full risk info available via /risk_v3."
            )
        else:
            final_msg = (
                f"✅ Thank you, {name}! Your V3 licence has been created for your bot.\n"
        ...
        if lang == "hi":
            error_msg = f"❌ एमटी5 आईडी {mt5} पहले से मौजूद है। कृपया नई आईडी के साथ /get_licence_v3 से प्रयास करें।"
        else:
            error_msg = f"❌ MT5 ID {mt5} already exists. Please try again with a different ID using /get_licence_v3."
...
licence_conv_v3 = ConversationHandler(
    entry_points=[CommandHandler("get_licence_v3", licence_start)],
    states={
        SHOW_ACCOUNT_INFO: [
